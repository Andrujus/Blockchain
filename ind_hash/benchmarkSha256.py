...
def measure_time(data: bytes, repeats=5):
    """Pamatuoja SHA-256 hashavimo laiką, grąžina vidurkį (sekundėmis)."""
    times = []
...
def main():
    ...
    plt.ylabel("Vidutinis SHA-256 hashavimo laikas (s)")
    plt.title("SHA-256 efektyvumo matavimai")
    plt.grid(True)
    plt.xscale("log")
    plt.yscale("log")
    plt.savefig("sha256_efektyvumas.png", dpi=150)  # išsaugom paveiksliuką
    plt.show()
